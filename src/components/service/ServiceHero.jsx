...
import serviceImage from "../../assets/images/walking.webp";
...
function ServiceHero({ service }) {
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-white via-[#f7f7f9] to-[#f5f5f7] px-6 py-16 sm:py-20 lg:py-28">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-16 overflow-hidden lg:grid-cols-[0.9fr_1.1fr]">
        <div>
          <Link
            to="/services"
          ...
          <div className="mt-10">
            <ExpandingCTA href="/contact">{service.cta}</ExpandingCTA>
          </div>
        </div>
        ...
        <div className="relative [perspective:1400px]">
          <MouseParallax strength={8}>
            <RevealImage
              src={serviceImage}
              alt={`${service.title} at Ultimate Health Men`}
              from="right"
              className="relative h-[420px] overflow-hidden rounded-[3rem] shadow-2xl sm:h-[520px] lg:h-[620px]"
            />
          </MouseParallax>
        </div>
      </div>
